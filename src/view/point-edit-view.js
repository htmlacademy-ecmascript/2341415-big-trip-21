...
function createPointTypeTemplate(type, {isChecked, isDisabled}) {

  const typeCammelCase = typeToCebabCase(type);

  return `<div class="event__type-item">
  <input id="event-type-${typeCammelCase}-1" class="event__type-input  visually-hidden" type="radio" name="event-type" value="${typeCammelCase}" ${isChecked ? 'checked' : ''} ${isDisabled ? 'disabled' : ''}>
  <label class="event__type-label  event__type-label--${typeCammelCase}" for="event-type-${typeCammelCase}-1">${type}</label>
  </div>`;
}

function createOffersTemplate({ title, price, id }, {isChecked, isDisabled}) {
  const cebabCaseTitle = title.split(' ').map((word) => word.toLowerCase()).join('-');
  const elementId = `event-offer-${cebabCaseTitle}-1`;
  return `<div class="event__offer-selector">
  <input class="event__offer-checkbox  visually-hidden" id="${elementId}" type="checkbox" name="event-offer-${cebabCaseTitle}" ${isChecked ? 'checked' : ''} value="${id}" ${isDisabled ? 'disabled' : ''}>
...
function createPointEditTemplate({ point }, state) {
  ...
  const { type, offers, destination, isSaving, isDeleting, isDisabled } = state;
  ...
  <form class="event event--edit" action="#" method="post">
    <header class="event__header">
      <div class="event__type-wrapper">
        ...
        <input class="event__type-toggle  visually-hidden" id="event-type-toggle-1" name="event-type" type="checkbox" ${isDisabled ? 'disabled' : ''}>

        <div class="event__type-list">
          <fieldset class="event__type-group">
            <legend class="visually-hidden">Event type</legend>
            ${Object.values(POINT_TYPES).map((pointType) => createPointTypeTemplate(pointType, { isChecked: pointType === type, isDisabled })).join('')}
          </fieldset>
        </div>
      </div>
      ...
      <div class="event__field-group  event__field-group--destination">
        ...
        <input class="event__input  event__input--destination" id="event-destination-1" list="edit-form-destination-list" required="value" id="event-destination-1" name="event-destination-1" type="text" value="${city}" ${isDisabled ? 'disabled' : ''}>
      ...
      <div class="event__field-group  event__field-group--time">
        ...
        <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${format(dateFrom, 'dd/MM/yy HH:mm')}" ${isDisabled ? 'disabled' : ''}>
        &mdash;
        <label class="visually-hidden" for="event-end-time-1">To</label>
        <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${format(dateTo, 'dd/MM/yy HH:mm')}" ${isDisabled ? 'disabled' : ''}>
      </div>
      ...
      <div class="event__field-group  event__field-group--price">
        ...
        <input type="number" class="event__input  event__input--price" id="event-price-1" type="text" name="event-price" min="${MIN_PRICE}" max="${MAX_PRICE}" value="${basePrice}" ${isDisabled ? 'disabled' : ''}>
      </div>
    ...
    <section class="event__details">
      <section class="event__section  event__section--offers">
        ...
        ${offers.map((offer) => createOffersTemplate(offer, {isChecked: selectedOfferIds.includes(offer.id), isDisabled}))}
        </div>
      </section>
...
export default class PointEditView extends FormView {
  ...
  constructor(params, listeners) {
    super(params, listeners);
    const { event, offersMap } = params;
    const {onFormSubmit, onDelete, onCancel} = listeners;
    this.event = event;
